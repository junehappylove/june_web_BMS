package com.june.controller.system.login;
		...
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){
				...
				if(Tools.isEmpty(errInfo)){
					errInfo = Const.SUCCESS;					//验证成功
				}
			}
		}else{
			errInfo = Const.ERROR;	//缺少参数
		}
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu){
		...
		try{
			...
			if (user != null) {
				...
				if(null == session.getAttribute(Const.SESSION_MENU_LIST) || ("yes".equals(changeMenu))){
					...
					if("2".equals(session.getAttribute("changeMenu"))){
						session.setAttribute(Const.SESSION_MENU_LIST, menuList1);
						session.removeAttribute("changeMenu");
						session.setAttribute("changeMenu", "1");
						menuList = menuList1;
					}else{
						session.setAttribute(Const.SESSION_MENU_LIST, menuList2);
						session.removeAttribute("changeMenu");
						session.setAttribute("changeMenu", "2");
						menuList = menuList2;
					}
				}else{
					menuList = (List<Menu>)session.getAttribute(Const.SESSION_MENU_LIST);
				}
				//切换菜单=====
