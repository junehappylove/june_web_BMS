package com.june.controller.weixin.textmsg;
...
import com.june.service.weixin.command.CommandService;
import com.june.service.weixin.imgmsg.ImgmsgService;
import com.june.service.weixin.textmsg.TextmsgService;
import com.june.util.AppUtil;
import com.june.util.Const;
import com.june.util.Jurisdiction;
import com.june.util.ObjectExcelView;
...
public class TextmsgController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		if(!Jurisdiction.buttonJurisdiction(menuUrl, Const.ADD)){return null;}
	...
	public void delete(PrintWriter out){
		...
		if(!Jurisdiction.buttonJurisdiction(menuUrl, Const.DEL)){return;}
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
	...
	public ModelAndView edit() throws Exception{
		...
		if(!Jurisdiction.buttonJurisdiction(menuUrl, Const.EDIT)){return null;}
		ModelAndView mv = this.initMV();
		PageData pd = new PageData();
		pd = this.getPageData();
	...
	public ModelAndView list(Page page){
		...
		if(!Jurisdiction.buttonJurisdiction(menuUrl, Const.CHA)){return null;}
		ModelAndView mv = this.initMV();
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
	...
	public ModelAndView goAdd(){
		...
		try {
			...
			mv.addObject(Const.MESSAGE, "save");
			mv.addObject(Const.PAGE_DATA, pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
		}						
	...
	public ModelAndView goEdit(){
		...
		try {
			...
			mv.setViewName("weixin/textmsg/textmsg_edit");
			mv.addObject(Const.MESSAGE, Const.EDIT);
			mv.addObject(Const.PAGE_DATA, pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
		}						
	...
	public Object deleteAll() {
		...
		if(!Jurisdiction.buttonJurisdiction(menuUrl, Const.DEL)){return null;}
		PageData pd = new PageData();		
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				pd.put(Const.MESSAGE, Const.OK);
			}else{
				pd.put(Const.MESSAGE, Const.NO);
			}
			pdList.add(pd);
			map.put("list", pdList);
		} catch (Exception e) {
	...
	public Object hasK(){
		...
		map.put(Const.RESULT, errInfo);				//返回结果
