...
public class Jurisdiction {
	...
	public static boolean hasJurisdiction(String menuUrl){
		...
		//Boolean b = true;
		List<Menu> menuList = (List<Menu>)session.getAttribute(Const.SESSION_ALL_MENU_LIST); //获取菜单列表
			...
			for(int j=0;j<menuList.get(i).getSubMenu().size();j++){
				if(menuList.get(i).getSubMenu().get(j).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])){
					if(!menuList.get(i).getSubMenu().get(j).isHasMenu()){				//判断有无此菜单权限
						...
						map.remove(Const.ADD);
						map.remove(Const.DEL);
						map.remove(Const.EDIT);
						map.remove(Const.CHA);
						String MENU_ID =  menuList.get(i).getSubMenu().get(j).getMENU_ID();
						String USERNAME = session.getAttribute(Const.SESSION_USERNAME).toString();	//获取当前登录者loginname
						Boolean isAdmin = "admin".equals(USERNAME);
						map.put(Const.ADD, (RightsHelper.testRights(map.get("adds"), MENU_ID)) || isAdmin?"1":"0");
						map.put(Const.DEL, RightsHelper.testRights(map.get("dels"), MENU_ID) || isAdmin?"1":"0");
						map.put(Const.EDIT, RightsHelper.testRights(map.get("edits"), MENU_ID) || isAdmin?"1":"0");
						map.put(Const.CHA, RightsHelper.testRights(map.get("chas"), MENU_ID) || isAdmin?"1":"0");
	...
	public static boolean buttonJurisdiction(String menuUrl, String type){
		...
		Session session = currentUser.getSession();
		//Boolean b = true;
		List<Menu> menuList = (List)session.getAttribute(Const.SESSION_ALL_MENU_LIST); //获取菜单列表
