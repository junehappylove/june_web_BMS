...
<div id="page-content">
  ...
  <div class="row-fluid">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function addRole(){
			 top.loading();
			 var URL = '${ctx}/role/toAdd.do?parent_id=0';
			 iscas.dialog("新增组", URL, 222, 90);
			 /* var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增组";
			 diag.URL = '${ctx}/role/toAdd.do?parent_id=0';
			 diag.Width = 222;
			 diag.Height = 90;
		...
		function addRole2(pid){
			 top.loading();
			 var URL = '${ctx}/role/toAdd.do?parent_id='+pid;
			 iscas.dialog("新增角色", URL, 222, 132);
			 /* var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增角色";
		...
		function editRole(ROLE_ID){
			 top.loading();
			 var URL = '${ctx}/role/toEdit.do?ROLE_ID='+ROLE_ID;
			 iscas.dialog("编辑", URL, 222, 132);
			 /* var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
		...
		function kf_qx1(id,kefu_id,msg){
			...
			if(qxhc1 == 'checked'){
				...
				var url = "${ctx}/role/kfqx.do?kefu_id="+kefu_id+"&msg="+msg+"&value="+value+"&guid="+new Date().getTime();
				$.get(url,function(data){
					if(data=="success"){
						//document.location.reload();
					}
		...
		function c1(id,msg,value,kefu_id){
				...
				var url = "${ctx}/role/gysqxc.do?kefu_id="+kefu_id+"&msg="+msg+"&value="+value+"&guid="+new Date().getTime();
				$.get(url,function(data){
					if(data=="success"){
						//document.location.reload();
					}
		...
		function editRights(ROLE_ID){
			 top.loading();
			 var URL = '${ctx}/role/auth.do?ROLE_ID='+ROLE_ID;
			 iscas.dialog("菜单权限", URL, 280, 370);
			/*  var diag = new top.Dialog();
			 diag.Drag = true;
			 diag.Title = "菜单权限";
			 diag.URL = '${ctx}/role/auth.do?ROLE_ID='+ROLE_ID;
			 diag.Width = 280;
			 diag.Height = 370;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function roleButton(ROLE_ID,msg){
			...
			var URL = '${ctx}/role/button.do?ROLE_ID='+ROLE_ID+'&msg='+msg;
			iscas.dialog(Title, URL, 200, 370);
			 
			 /* var diag = new top.Dialog();
			 diag.Drag = true;
			 diag.Title = Title;
			 diag.URL = '${ctx}/role/button.do?ROLE_ID='+ROLE_ID+'&msg='+msg;
			 diag.Width = 200;
			 diag.Height = 370;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
