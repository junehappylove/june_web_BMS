...
<body>
											...
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													<td class='center' style="width: 30px;"><label><input
															type='checkbox' name='ids' value="${var.PICTURES_ID}" /><span
															class="lbl"></span></label></td>
													<td class='center' style="width: 30px;">${vs.index+1}</td>
													<td><a
														href="${ctx}/uploadFiles/uploadImgs/${var.PATH}"
														title="${var.TITLE}" class="bwGal"><img
															src="${ctx}/uploadFiles/uploadImgs/${var.PATH}"
													...
													<td>${var.MASTER_ID}</td>
													<td>${var.BZ}</td>
													<td style="width: 30px;" class="center"><c:if
	...
	<script type="text/javascript">
		...
		function add() {
			top.loading();
			var URL = '${ctx}/pictures/goAdd.do';
			iscas.dialog("新增", URL, 800, 490, function() {
		...
		function del(Id, PATH) {
			if (confirm("确定要删除?")) {
				top.loading();
				var url = "${ctx}/pictures/delete.do?PICTURES_ID=" + Id
						+ "&PATH=" + PATH + "&tm=" + new Date().getTime();
		...
		function edit(Id) {
			top.loading();
			var URL = '${ctx}/pictures/goEdit.do?PICTURES_ID=' + Id;
			iscas.dialog("编辑", URL, 600, 465, function() {
				...
				for (var i = 0; i < document.getElementsByName('ids').length; i++) {
					if (document.getElementsByName('ids')[i].checked) {
						if (str == '')
							...
							str += ','
									+ document.getElementsByName('ids')[i].value;
					}
				}
				if (str == '') {
					alert("您没有选择任何内容!");
					...
					if (msg == '确定要删除选中的数据吗?') {
						...
						$.ajax({
							type : "POST",
							url : '${ctx}/pictures/deleteAll.do?tm='
									+ new Date().getTime(),
							data : {
								DATA_IDS : str
							},
